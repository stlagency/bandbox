...
| **M4** | Serving + map. **Read APIs DONE + live-verified** against real M3 data: `/api/scan` (4 lenses, quantile buckets, period ranges), `/api/parcel/:pk` (full deep-dive bundle), `/api/comps` (deterministic ladder + transparent estimate), `/api/leads` (scored/filtered/paginated), `/api/boundaries` (GeoJSON). **MapLibre choropleth (`ScanMap`) wired** to the live APIs (real neighborhoods, lens-colored, theme-aware) — builds clean, no runtime errors; pixel render unverified (headless preview viewport is 0×0 — confirm via `pnpm --filter @phillybricks/web dev`). **Remaining (needs Vercel Pro + R2):** deploy + nightly PMTiles→R2 + the high-zoom parcel layer | 🔄 **APIs+map done; deploy/tiles need creds** |
