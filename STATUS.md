...
Ingestion-first per PRD §9. State history (change-logs) only accrues forward and is the one irrecoverable asset. **→ Live at https://phillybricks.vercel.app. Resume point: [`docs/NEXT_SESSION.md`](docs/NEXT_SESSION.md) — finish M4: nightly PMTiles→R2 + high-zoom parcel layer (needs R2 keys, in progress) + map polish (time control, filters, real right-rail).**
