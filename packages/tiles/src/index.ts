 ...
 * the tiny static aggregate-boundary tiles, and uploads them to Cloudflare R2
 ...
 *                                            ├─► ndjson GeoJSON ─► tippecanoe ─► *.pmtiles ─► R2
 *   public.geo_boundary (zip/nbhd/tract) ───┘
 *
 * tippecanoe MUST be installed in the CI/runner image (see build.ts header).
 * Reads DATABASE_URL + R2_* from process.env — no secrets in source (PRD §0.3, §8).
 */
...
export type { R2Config, TileUploadResult } from './r2.js';
